...
class DQN(object):
    ...
    def get_action(self, s_t):
        ...
        if np.random.rand() < self.ep:#NOTE:random explore
            action = np.random.randint(0, self.action_dim)
        else:#NOTE:cacu action 
            action = np.argmax(self.sess.run(self.q_action, feed_dict={self.state: s_t}))#NOTE:one-hot code
        return action

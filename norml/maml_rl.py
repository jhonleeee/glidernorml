...
from glidernorml.norml import rollout_service
from glidernorml.norml.tools import utility
...
class MAMLReinforcementLearning(object):
  ...
  def __init__(self, config, reporter=None, logdir=None, save_config=False):
  ...
  def _configure(self):
    ...
    if self.save_config:#TODO: have bug
  ...
  def train(self,
    ...
    for step in range(self.current_step,
                      self.current_step + num_outer_iterations):#from config

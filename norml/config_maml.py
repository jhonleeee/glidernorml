...
from glidernorml.norml.envs import move_point_env
#import sender config
from glidernorml.glider.config import EnvironmentConfig
from glidernorml.glider.env import sender
...
CARTPOLE_SENSOR_MAML_OFFSET.update(
    learn_offset=True,#use offSet to borden explors
    inner_lr_init=1e-1,
    outer_lr_init=4e-4,
...
#glider config-modified from cartpole config
SENDER_ENV_CONFIG = EnvironmentConfig
GLIDER_CC_DR = dict(
    random_seed=random.randint(0, 1000000),
    num_outer_iterations=1000,
    task_generator=functools.partial(
        sender.Sender,SENDER_ENV_CONFIG,1600),
    #Sender(SENDER_ENV_CONFIG,1600),
  task_env_modifiers=[{
        '_action_rotation': i
    } for i in np.linspace(-np.pi, np.pi, 5000)],
    network_generator=networks.FullyConnectedNetworkGenerator(
        ...
        activation_fn=tf.nn.tanh),
    input_dims=4,#state_dim
    pol_log_std_init=-4.0,
    output_dims=5,#action_dim
    reward_disc=0.97,
    learn_offset=False,
    policy=policies.GaussianPolicy,
    tasks_batch_size=10,
    num_inner_rollouts=1,#train how many envs inner parales
    outer_optimizer_algo=tf.train.AdamOptimizer,
    advantage_function='returns-values',
    whiten_values=False,
    always_full_rollouts=False,
...
GLIDER_CC_MAML_OFFSET = GLIDER_CC_MAML.copy()
CARTPOLE_SENSOR_MAML_OFFSET.update(
    learn_offset=True,
    inner_lr_init=1e-1,
    outer_lr_init=4e-4,
...
GLIDER_CC_MAML_LAF = GLIDER_CC_MAML.copy()
GLIDER_CC_MAML_LAF.update(
    learn_advantage_function_inner=True,
    advantage_generator=networks.FullyConnectedNetworkGenerator(
        dim_input=2 * 4 + 5,
        dim_output=5,
...
GLIDER_CC_NORML = GLIDER_CC_MAML_LAF.copy()
GLIDER_CC_NORML.update(
    learn_offset=True,
    inner_lr_init=7e-4,
    outer_lr_init=3e-4,
    pol_log_std_init=-3.5)
...
class Test(object):
    def __init__(self, args, run_id, cc, cc_schmes=None):
        self.mode = args.mode
        self.run_id = run_id
        self.cc = cc
        self.cc_schemes = cc_schmes
    ...
    def run_scc_first_side(self, cc_id, send_manager, recv_manager, send_pri_ip, recv_pri_ip):
        first_src = path.join(context.src_dir, 'wrappers', self.cc_schemes[cc_id-1] + '.py')
        second_src = first_src

        self.run_first, self.run_second = utils.who_runs_first(self.cc_schemes[cc_id-1])
        self.r = utils.parse_remote_path(args.remote_path, self.cc_schemes[cc_id-1])
        if self.run_first == 'receiver':
            if self.mode == 'remote':
                if self.sender_side == 'local':
                    first_src = self.r['cc_src']
    ...
    def run_scc_second_side(self, send_manager, recv_manager, second_cmds):
        time.sleep(self.run_first_setup_time)
    ...
    def run_scc_with_tunnel(self):
        ...
        if self.sender_side == self.server_side:
         ...
         # run every congestion control flow
        second_cmds = []
        for cc_id in xrange(1, len(self.cc)+1):
            # run tunnel server for tunnel cc_id
            cmd_to_run_tc = self.run_tunnel_server(cc_id, ts_manager)

            # run tunnel client for tunnel cc_id
            if not self.run_tunnel_client(cc_id, tc_manager, cmd_to_runtc):
                return False
            ...
            # run the side that runs first and get cmd to run the other side
            second_cmd = self.run_scc_first_side(
                tun_id, send_manager, recv_manager, send_pri_ip, recv_pri_ip)
            second_cmds.append(second_cmd)

         # run the side that runs second
        if not self.run_scc_second_side(send_manager, recv_manager, second_cmds):
            return False
    ...
    def run_scc(self):
        msg = 'Testing scheme simultaneously for experiment run %d/%d...' % (self.run_id, self.run_times)
        sys.stderr.write(msg + '\n')
        ...
        if self.flow > 0:
            try:
                return self.run_scc_with_tunnel()
            finally:
                utils.kill_proc_group(self.ts_manager)
                utils.kill_proc_group(self.tc_manager)
...
def run_tests(args):
    ...
    for run_id in xrange(args.start_run_id,
        ...
        # run congestion control simultaneously
        if args.scc:
            Test(args, run_id, cc_schemes).run_scc()
        elif not hasattr(args, 'test_config') or args.test_config is None:
